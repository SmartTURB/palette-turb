...
class Palette(BaseModel):
    ...
    @staticmethod
    def create_signal_figure(sig_tensor, ncols=4):
        shape = sig_tensor.shape
        nrows = int(np.ceil(shape[0] / ncols))
        ...
        for i in range(shape[0]):
            row = i // ncols
            col = i % ncols
            ax = fig.add_axes([col*ax_width, 1 - (row+1)*ax_height, ax_width, ax_height])
            for j in range(shape[1]):
                ax.plot(sig_tensor[i, j, :], lw=0.5)
            ax.set_xlim(0, 2000)
    ...
    def save_current_turb_results(self):
        ret_name = []
        ret_result = []

        ret_name.append('gt_image')
        ret_result.append(self.gt_image)

        ret_name.append('output')
        ret_result.append(self.output)

        if self.task in ['inpainting','uncropping']:
            ret_name.append('mask')
            ret_result.append(self.mask)

        self.results_dict = self.results_dict._replace(name=ret_name, result=ret_result)
        return self.results_dict._asdict()
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            if self.iter % self.opt['train']['log_iter'] == 0:
                ...
                dims = self.netG.module.dims if self.opt['distributed'] else self.netG.dims
                for key, value in self.get_current_visuals().items():
                    if dims == 1:
                        self.writer.add_figure(key, Palette.create_signal_figure(value))
                    elif dims == 2:
            ...
            for val_data in tqdm.tqdm(self.val_loader):
                ...
                for key, value in self.get_current_visuals(phase='val').items():
                    if dims == 1:
                        ...
                        self.writer.add_images(key, value)
                if dims == 1:
                    self.writer.save_signals(self.save_current_results())
                elif dims == 2:
        ...
        with torch.no_grad():
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                if dims == 1:
                    ...
                    self.writer.save_images(self.save_current_results())
                self.writer.save_turb_data(self.save_current_turb_results())
