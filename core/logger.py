...
class VisualWriter():
    ...
    def save_signals(self, results):
        ...
        ''' get names and corresponding signals from results[OrderedDict] '''
        try:
            names = results['name']
            outputs = Util.postprocess_signals(results['result'])
            for i in range(len(names)): 
               outputs[i].savefig(os.path.join(result_path, names[i]))
        except:
            raise NotImplementedError('You must specify the context of name and result in save_current_results functions of model.')

    def save_turb_data(self, results):
        result_path = os.path.join(self.result_dir, self.phase)
        os.makedirs(result_path, exist_ok=True)
        result_path = os.path.join(result_path, str(self.epoch))
        os.makedirs(result_path, exist_ok=True)

        ''' get name and corresponding turbulent data from results[OrderedDict] '''
        for name, output in zip(results['name'], results['result']):
            if self.distributed:
                outputs_list = [torch.zeros_like(output) for _ in range(dist.get_world_size())]
                dist.all_gather(tensor_list=outputs_list, tensor=output)
            else:
                outputs_list = [output]
            combined_output = torch.cat(outputs_list, dim=0)

            n_dim = combined_output.dim()
            if n_dim == 4:
            ...
            combined_output = combined_output.permute(permute_order).cpu().numpy()

            if self.rank == 0:
                with h5py.File(os.path.join(result_path, 'turb_data.h5'), 'a') as hf:
                    if name not in hf:
                        maxshape = (None,) + combined_output.shape[1:]
                        dset = hf.create_dataset(name, data=combined_output, maxshape=maxshape)
                    else:
                        dset = hf[name]
                        dset.resize((dset.shape[0] + combined_output.shape[0],) + combined_output.shape[1:])
                        dset[-combined_output.shape[0]:] = combined_output
