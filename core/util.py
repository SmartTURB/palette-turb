...
def create_signal_figure(sig_tensor, ncols=1, scale=3):
	if sig_tensor.dim() == 2:
		sig_tensor = sig_tensor.unsqueeze(0)
	shape = sig_tensor.shape
	nrows = int(np.ceil(shape[0] / ncols))
	fig = plt.figure(figsize=(scale*ncols*2, scale*nrows))
	ax_width = 1 / ncols
	ax_height = 1 / nrows
...
def postprocess_signals(signals):
	return [create_signal_figure(signal) for signal in signals]
